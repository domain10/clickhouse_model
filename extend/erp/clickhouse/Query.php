...
class Query
{
    ...
    {
        $this->connection = $connection ?: Db::connect(Config::get('ch_config'), true);
...
//         }
        if (false === $result) {
            ...
            $data = $this->field($field)->limit(1)->getColumn(0);
            $result = current($data);
            if ($force) {
...
//         }
        if (false === $result) {
            ...
            $data = $this->field($field)->getColumn();
            if (isset($data[0]) && 1 == count($data[0])) {
                $result = array_map(function($v){return current($v);}, $data);
            } else {
                $resultSet = $data;
                if ($resultSet) {
...
//             }
    ...
    {
        $this->parseWhereExp($logic, $field, 'isNull', null);
        return $this;
    }
    ...
    {
        $this->parseWhereExp($logic, $field, 'isNotNull', null);
        return $this;
    }
    ...
    public function getTableInfo($tableName = '', $fetch = '')
    {
        ...
        ($len = strpos($db, ',')) && ($db = substr($db, 0, $len));
    ...
    /**
     * 插入记录--(不可在生成环境使用，要用 insertAll 进行大量批插入)
     * @access public
     * @param mixed   $data         数据
     ...
     * @return array
     */
    public function getColumn(int $c = -1)
    {
        // 分析查询表达式
        $options = $this->parseExpress();
        // 生成查询SQL
        ...
        // 执行查询操作
        $res = $this->query($sql, $bind, $options['master'], true);
        return $res[$c] ?? $res;
    }
