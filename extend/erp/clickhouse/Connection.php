...
class Connection
{
    ...
    protected $config = [
        ...
        'sql_explain'     => false,
        // Builder类
        'builder'         => '\erp\clickhouse\Builder',
        // Query类
        'query'           => '\erp\clickhouse\Query',
        // 是否需要断线重连
    ...
    public function connect(array $config = [], $linkNum = 0, $autoConnection = false)
    {
        if (!isset($this->links[$linkNum])) {
            ...
            try {
                ...
                $this->links[$linkNum] = new Client($config, $config['settings']);
                if ($config['debug']) {
                    // 记录数据库连接信息
     ...
     * @throws PDOException
     */
    public function query($sql, $bind = [], $master = false, $pdo = false, $num = 0)
    {
        ...
        try {
            ...
            $this->debug(true);
            // 执行查询
            $result = $this->linkID->select($sql, $bind)->rows();
            // 调试结束
            $this->debug(false);
            // 返回结果集
            return $result;
        } catch (\Throwable $e) {
            if ($this->config['break_reconnect'] && $num < 2 && $this->isBreak($e)) {
                return $this->close()->query($sql, $bind, $master, $pdo, ++$num);
            }
     ...
     * @throws PDOException
     */
    public function execute($sql, $bind = [], $num = 0)
    {
        ...
        try {
            ...
            $this->debug(true);
            // 执行语句
            $res = $this->linkID->write($sql, $bind);
            // 调试结束
            $this->debug(false);
            ...
            if ($this->config['break_reconnect'] && $num < 2 && $this->isBreak($e)) {
                return $this->close()->execute($sql, $bind, ++$num);
            }
    ...
    {
        return ValueFormatter::formatValue($str);
        //$this->initConnect($master);
        //return $this->linkID ? $this->linkID->quote($str) : $str;
    }
    
    public function quoteTwo(string $str)
    {
        return FormatLine::Insert([$str]);
    }
