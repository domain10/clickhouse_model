...
class Builder
{
    ...
    // 数据库表达式
    protected $exp = ['eq' => '=', 'neq' => '<>', 'gt' => '>', 'egt' => '>=', 'lt' => '<', 'elt' => '<=', 'notlike' => 'NOT LIKE', 'like' => 'LIKE', 'in' => 'IN', 'exp' => 'EXP', 'notin' => 'NOT IN', 'not in' => 'NOT IN', 'between' => 'BETWEEN', 'not between' => 'NOT BETWEEN', 'notbetween' => 'NOT BETWEEN', 'exists' => 'EXISTS', 'notexists' => 'NOT EXISTS', 'not exists' => 'NOT EXISTS', 'null' => 'Null', 'notnull' => 'NotNull', 'not null' => 'NotNull', '> time' => '> TIME', '< time' => '< TIME', '>= time' => '>= TIME', '<= time' => '<= TIME', 'between time' => 'BETWEEN TIME', 'not between time' => 'NOT BETWEEN TIME', 'notbetween time' => 'NOT BETWEEN TIME'];

    // SQL表达式
    protected $selectSql    = 'SELECT%DISTINCT% %FIELD% FROM %TABLE%%FORCE%%JOIN% %WHERE%%GROUP%%HAVING%%ORDER%%LIMIT% %UNION%';
    protected $insertSql    = '%INSERT% INTO %TABLE% (%FIELD%) VALUES (%DATA%)';
    protected $insertAllSql = 'INSERT INTO %TABLE% (%FIELD%) VALUES %DATA%';
    protected $updateSql    = 'alter table %TABLE% update %SET% %JOIN% %WHERE% %ORDER%';
    protected $deleteSql    = 'alter table %TABLE% delete %USING% %JOIN% %WHERE% %ORDER%';
...
//             } else
            if (isset($val[0]) && 'exp' == $val[0]) {
                ...
                if (0 === strpos($val, ':') && $this->query->isBind(substr($val, 1))) {
                    ...
                    $result[$item] = $this->connection->quoteTwo($val);
                }
            } elseif (is_object($val) && method_exists($val, '__toString')) {
                // 对象数据写入
                $result[$item] = $this->connection->quoteTwo($val->__toString());
            }
        }
        return $result;
    }
    ...
    protected function parseWhereItem($field, $val, $rule = '', $options = [], $binds = [], $bindName = null)
    {
        ...
        if (is_scalar($value) && array_key_exists($field, $binds) && !in_array($exp, ['EXP', 'NotNull', 'Null', 'IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN']) && strpos($exp, 'TIME') === false) {
        ...
        if (in_array($exp, ['=', '<>', '>', '>=', '<', '<='])) {
            ...
            // 表达式查询
            $whereStr .= '( ' . $key . ' ' . $value . ' )';
        } elseif (in_array($exp, ['NotNull', 'Null'])) {
            // NULL 查询
            $whereStr .= 'is' . $exp . '('. $key .')';
